...
export class IslandPIXI {
  ...
  imageStringToContainer(string, options) {
    const container = new PIXI.Container();
    var imageArray = string.split(";");
    for (let i in imageArray) {
      ...
      if (options.offset !== null) {
        sprite.position.x = spriteData.xPos * 4 + options.offset.x;
        sprite.position.y = spriteData.yPos * 4 + options.offset.y;
      }
      if (options.selectedShape == i) {
        sprite.filters = [this.selectedFilter];
      }
      let mirror = 1;
  ...
  loadAssets() {
    ...
    this.overrideDefaultHitCheckWithPixelPerfectCheck();
  }
